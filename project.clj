(defproject kafka-toy "1.0.0-SNAPSHOT"
  :description "Kafka-toy service"
  ...
  :ring {:handler kafka-toy.web/app
         :main kafka-toy.setup
         :port ~(Integer/valueOf (get (System/getenv) "SERVICE_PORT" "8080"))
         :init kafka-toy.setup/setup
         :browser-uri "/healthcheck"
         :nrepl {:start? true}}

  :uberjar-name "kafka-toy.jar"

  :rpm {:name "kafka-toy"
        :summary "RPM for Kafka-toy service"
        ...
        :mappings [{:directory "/usr/local/kafka-toy"
                    :filemode "444"
                    :username "kafka-toy"
                    :groupname "kafka-toy"
                    :sources {:source [{:location "target/kafka-toy.jar"}]}}
                   {:directory "/usr/local/kafka-toy/bin"
                    :filemode "744"
                    :username "kafka-toy"
                    :groupname "kafka-toy"
                   ...
                   {:directory "/etc/rc.d/init.d"
                    :filemode "755"
                    :sources {:source [{:location "scripts/service/kafka-toy"
                                        :destination "kafka-toy"}]}}]}
