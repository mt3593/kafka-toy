...
then
  /bin/echo "Kafka-toy is not running"
else
  sleepCounter=0
  sleepIncrement=2
  waitTimeOut=600
  ...
  /bin/echo "Kafka-toy is running, stopping service"
...
then
  mkdir -p $prefixDir
  /usr/sbin/useradd -r -s /sbin/nologin -d $prefixDir -m -c "Kafka-toy user for the Kafka-toy service" $APP_NAME 2> /dev/null || :
fi
