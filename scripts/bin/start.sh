...
if [ -n "$PIDS" ]
then
  echo "Kafka-toy is already running in process $PIDS";
  exit 1
fi
...
LOG_FILE=${LOGGING_PATH}/kafka-toy.out
ERR_FILE=${LOGGING_PATH}/kafka-toy.err
...
echo "Giving Kafka-toy $waitTimeout seconds to start successfully"
...
do
  ...
  then
    echo "Kafka-toy didn't start within $waitTimeout seconds."
    PIDS=$(pgrep java -lf | grep $APP_NAME | cut -d" " -f1);
    if [ -n "$PIDS" ]
	then
	  echo "Killing $PIDS";
...
if [ $retVal -eq 1 ]
then
  echo "Starting Kafka-toy failed"
else
  echo "Starting Kafka-toy succeeded"
fi
